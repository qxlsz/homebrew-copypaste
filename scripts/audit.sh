...
if grep -Eq '(shell_output|system|pipe_output)[([:space:]]+["'\'']copypaste[[:space:]"'\'']' <<<"${test_block}"
then
  fail 'fully scope test calls, e.g. shell_output("#{bin/"copypaste"} --help")'
fi

if grep -q '#{bin}/copypaste' "${FORMULA}"
then
...
if grep -q 'config init' <<<"${test_block}"
then
  :
elif grep -Eq '[[:space:]]send[[:space:]]|[[:space:]]serve[[:space:]]|healthcheck' <<<"${test_block}"
then
  :
else
